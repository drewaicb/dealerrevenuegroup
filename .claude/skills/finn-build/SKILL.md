...
Using the Linear connector, list issues on team `DEA` that meet every
condition:
...
- Fetch the latest default branch from `origin` and create or resume a branch
  named `DEA-NNN-short-slug`, using the issue's real identifier.
...
- `Closes DEA-NNN`, using the real Linear issue identifier
